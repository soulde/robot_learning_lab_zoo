...
DEEPROBOTICS_DR02_PRO_CFG = ArticulationCfg(
    ...
    soft_joint_pos_limit_factor=0.9,
    # Actuator parameters follow DeepRoboticsLab/rl_training's official
    # DR02 AMP config (RandomPDActuatorCfg values, without its domain
    # randomization). The official setup trains 21 DOFs and locks waist
    # roll/pitch plus all wrists; we keep the full 29 DOFs and give those
    # extra joints their neighbours' parameters (waist x/y <- waist_z,
    # wrists <- elbow). Arms/wrists keep the USD equivalent-inertia
    # armature, which the official config omits by fixing those joints.
    actuators={
        "waist": DCMotorCfg(
            joint_names_expr=["waist_[xyz]_joint"],
            effort_limit=105.0,
            saturation_effort=105.0,
            velocity_limit=17.38,
            stiffness=150.0,
            damping=3.0,
            friction=0.0,
        ),
        ...
        "wrists": DCMotorCfg(
            ...
            saturation_effort=50.0,
            velocity_limit=23.76,
            # Extra joints absent from the official config: borrow the
            # elbow's drive parameters.
            stiffness=100.0,
            damping=2.5,
            armature={
